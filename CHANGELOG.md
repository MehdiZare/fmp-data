...
- **`setup_registry()` no longer swallows non-validation errors** (#121) - It used to wrap each group's registration in `except Exception`, log an error, and continue. Validation failures are now handled per endpoint inside `register_batch()`, and anything else — an import error from the lazy endpoint-group loading, for instance — propagates instead of leaving a silently half-built registry. This is the loud-failure half of the #121 fix: a group that cannot load at all should not look like a group with nothing in it.
