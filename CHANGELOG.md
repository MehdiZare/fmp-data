...
### Added
- **Catalogue-wide LangChain endpoint↔method parameter coverage guard** (#188) - after #172 / #186, tools dispatch through `client.<client>.<method>` when every required method parameter can be filled from wire/endpoint fields, otherwise fall back to `client.request`. That gate is per-tool at create time; a new shape mismatch could still land only as a silent request-fallback or a mandatory wire field omitted from the tool schema. `tests/unit/lc/test_endpoint_method_coverage.py` walks every `(endpoint_map × semantics)` pair, requires each method to resolve on a live client, pins the two known request-fallback tools (`institutional.get_form_13f`, `institutional.get_institutional_holdings` — wire `year`/`quarter` vs method `report_date`), and pins the two known dropped-mandatory wire fields (revenue segmentation `structure`). Any *new* mismatch fails CI until allowlisted with a comment or fixed.
- **Claude Code Review soft-failure job summary and maintainer notes** (#187) - #184 / #185 skip the job when `CLAUDE_CODE_OAUTH_TOKEN` is empty and soft-fail when the token is present but broken, so product PRs stay green. Soft-fail was silent: an expired secret could hide missing reviews for months. The workflow now writes a step-summary checklist when the review step fails, and `.github/CLAUDE_CODE_REVIEW.md` (linked from `CONTRIBUTING.md`) documents how to rotate the OAuth token. Review remains advisory; required checks stay in `ci.yml`. Restoring the secret itself is an ops step for maintainers (Settings → Secrets).

### Changed
- **Document LangChain wire names vs MCP method parameter names** (#188) - LangChain tool schemas keep API/wire parameter names and map at invoke; MCP tools expose Python method parameter names. README (LangChain section) and `docs/mcp/index.md` now state the difference so callers do not treat one schema as wrong.
