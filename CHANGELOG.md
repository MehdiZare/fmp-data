...
- **Stale endpoint count in the API docs** (#146) - `docs/api/endpoints.md` claimed 47 Market Intelligence endpoints; the code and the document's own table both had 46. `docs/mcp/tools.md` has been guarded by `test_docs_tools_sync.py` for some time, which is why the catalog change in #130 could not leave a stale number behind; `endpoints.md` and `docs/mcp/configurations.md` had no equivalent. They do now — per-client counts and per-section table rows are asserted against the code, and every `examples/...` path the MCP configuration guide names must exist.
