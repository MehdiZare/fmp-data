...
### Fixed
- **A positional-only required parameter no longer passes the dispatch gate** (#195) - `bindable_params` lists parameters fillable *by name*, and dispatch is `method(**mapped)`. A required `POSITIONAL_ONLY` parameter was merely absent from that map, which read as "nothing required is missing": the gate reported the method compatible and the call then raised `TypeError: missing 1 required positional argument`, which is the exact failure the gate exists to prevent. New `keyword_unfillable_required_params()` reports them and `uncovered_required_params()` includes them, so such a method falls back to `client.request`. No catalog method has one today.
- **`bindable_params` drops `cls`, not just `self`** (#195) - a bound classmethod hides its receiver, but the catalogue guard and the unit tests pass the underlying function, where `cls` read as a required parameter no wire field could fill.
- **Conflicting wire aliases raise instead of silently picking one** (#195) - if two keys resolve to the same method parameter with *different* values (e.g. `from` and `from_date` against a method taking `from_date`), `map_tool_kwargs_to_method` now raises `ValueError` naming both. Last-wins would have quietly answered about the wrong date window. Agreeing values are not a conflict. Unreachable from a well-formed tool schema (`extra="forbid"`); this protects direct callers.
- **The async institutional client no longer swallows every exception** (#193) - `get_form_13f_by_quarter` and `get_form_13f_dates` caught bare `Exception` and reported "no 13F data", so a `TypeError` in this library surfaced as an empty portfolio. Narrowed to `(FMPError, ValidationError)`, matching the sync client. API errors still return an empty list.
- **`get_holder_performance_summary(report_date=...)` warns that it does nothing** (#192) - the FMP endpoint accepts `year`/`quarter` and ignores them. Probed live: `cik=0001067983` returns a byte-identical 52-row payload spanning 2013-06-30 to 2026-03-31 with no filter, with 2023 Q3, and with 2019 Q1. Callers were getting the holder's entire history while believing they had selected a period, so passing `report_date` now emits a `UserWarning` pointing at the `date` field on each row. The parameter is still sent in case FMP starts honouring it, and the method deliberately has **no** `_by_quarter` sibling — advertising a filter the API does not apply would hide the problem rather than expose it.
...
- **Wire-shaped `*_by_quarter` methods for the rest of the institutional year/quarter group** (#192) - `get_institutional_ownership_extract_by_quarter`, `get_institutional_ownership_analytics_by_quarter`, `get_holder_industry_breakdown_by_quarter`, `get_symbol_positions_summary_by_quarter` and `get_industry_performance_summary_by_quarter`, on both the sync and async clients. Each existing date-shaped method keeps its signature and becomes a thin delegate. All five were verified against the live API to actually honour `year`/`quarter` (a 2023 Q3 request returns 2023-09-30 rows, a 2019 Q1 request returns 2019-03-31 rows) — unlike `holder-performance-summary`, see **Fixed**. None of these five is an MCP or LangChain tool, so this is Python-API consistency only; no tool surface changes.
- **`MethodBinding` / `bind_client_method` report *why* dispatch fell back** (#194) - a bare `Callable | None` could not distinguish "this client has no such sub-client" (normal for a bare `BaseClient`) from "the semantics name a method that does not exist" or "the shape cannot be filled". `EndpointVectorStore.create_tool` now announces a fallback instead of taking it silently: DEBUG for the bare-client case, which is expected and would otherwise emit one line per endpoint in the catalogue, and WARNING naming the cause (and the uncovered parameters) for the two that mean something is actually misconfigured. `resolve_dispatch_method` is unchanged and still returns `Callable | None`.
...
- **A cache TTL override naming no endpoint now warns instead of silently doing nothing** (#166, follow-up) - `BaseClient._get_cache_ttl` reads `CacheConfig.ttl_overrides` with a plain `.get(endpoint_name, default)`, so a key matching no endpoint was not an error, not a warning and not visible. `CacheConfig(ttl_overrides={"serch-name": 60})` was accepted in full and the override simply never applied; the only symptom was request volume quietly disagreeing with the configuration. A typo, a name copied from a newer version, or a name the library has since changed all failed the same silent way.
  ...
  - Found while establishing the blast radius of #166, which proposes renaming `market.search-name`. That rename was blocked partly because it would silently orphan a user's TTL override; with this in place the loss is announced rather than silent. **#166 itself is deferred to 3.0** — the rename also orphans persisted entries in the `file` and `redis` cache backends, since `_build_cache_key` uses `Endpoint.name` as a literal key prefix, and neither breakage earns a minor release.
...
- **`docs/api/endpoints.md` documented endpoints under names and paths the code does not use** (#146) - `docs/mcp/tools.md` has been guarded by `test_docs_tools_sync.py` for some time, which is why the catalog change in #130 could not leave a stale number behind; `endpoints.md` and `docs/mcp/configurations.md` had no equivalent, and both had drifted. The document is now asserted against the code row for row, not just in aggregate.
  ...
  - **Row-set equality, because a rename miscounts as zero.** Three rows named endpoints that do not exist: `search_name` (the endpoint is `search-name`), `crypto_symbol_news` and `forex_symbol_news` (they are `crypto_news_symbol` and `forex_news_symbol`). Every count in those two sections matched, so nothing could have noticed
  ...
  - **The code is the authority, so every correction landed in the document.** `Endpoint.name` is not cosmetic — it is the cache-key prefix and the argument to `_get_cache_ttl` — so renaming `search-name` to match its documentation would have quietly changed cache behaviour. That naming oddity is filed separately
