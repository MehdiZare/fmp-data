...
- **`ETFHoldingsArgs.date` is no longer required, and arg models are now guarded against being stricter than their endpoints** (#143) - `ETF_HOLDINGS` declares `date` an *optional* parameter while `ETFHoldingsArgs` declared it *required*, so a caller who satisfied the endpoint's contract was rejected by the tool schema before a request was ever built. The endpoint was right: probed against the live API on 2026-08-08, `etf/holdings?symbol=SPY` returns 505 rows with *and* without `date`. The field is now `dt_date | None = None`.
  ...
  - `tests/unit/test_arg_model_consistency.py` is the new guard: no arg-model field may be required where its endpoint declares the parameter optional. It scans every endpoint carrying an `arg_model`, so newly-wired ones are covered automatically, and checks the two investment holdings models explicitly since they are not wired yet
  - Only that one direction is treated as a defect. An arg-model field that is *optional* against a *mandatory* endpoint param cannot under-supply the request — a pydantic field is optional exactly when it has a default — so `LatestFinancialStatementsArgs.page` defaulting to `0` against a mandatory `page` is correct and is not flagged
