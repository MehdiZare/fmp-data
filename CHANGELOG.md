...
### Changed
- **Institutional 13F LangChain and MCP tools now take `year`/`quarter`** (#188) - the `form_13f` and `institutional_holdings` semantics point at the new wire-shaped methods, so the request-fallback allowlist in `tests/unit/lc/test_endpoint_method_coverage.py` is now **empty**: every tool in the catalog dispatches through a client method. Two user-visible effects for tool callers (the Python API is untouched):
  - The **MCP** tools keep their names (`form_13f`, `institutional_holdings`) but their arguments change from `cik`/`symbol` + `report_date` to `cik`/`symbol` + `year` + `quarter`. These now match the `parameter_hints` those semantics entries already declared, and an assistant no longer has to invent a quarter-end date to ask for a quarter.
  - The **LangChain** tools are renamed from `get_form_13f` / `get_institutional_holdings` to `get_form_13f_by_quarter` / `get_institutional_holdings_by_quarter` (LangChain tool names come from `EndpointSemantics.method_name`). Their argument schemas are unchanged — `cik`/`symbol`, `year`, `quarter` — because these tools already used the wire schema under request-fallback.
