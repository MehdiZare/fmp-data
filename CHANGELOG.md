...
- **CI: automation PRs trigger normal CI; release PR health re-checked; sync hardened** (#206, #207, #208).
  ...
  - **#207 — Release-PR re-validates an already-open `dev → main` PR on every push.** The old “already open → exit 0” path skipped ancestry and mergeability checks, so a release PR that went CONFLICTING after a squash stayed green on every later `dev` push. Ancestry plus REST `mergeable` / `mergeable_state` (with retries) now fail the job on `dirty`, `mergeable=false`, or still-`unknown` after retries — same fail-closed contract as Guard-Main-Origin (GraphQL `mergeStateStatus` is uppercase and was easy to mis-match).
