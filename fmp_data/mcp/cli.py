...
def _manifest_header(excluded: list[str], collisions: dict[str, list[str]]) -> str:
...
def _startable_catalog(available_specs: set[str], excluded: list[str]) -> list[str]:
    ...
    both be registered. Each removal appends a note to ``excluded``, which the
    file header prints, so nothing disappears silently.
    """
    from fmp_data.mcp.tools_manifest import DEPRECATED_TOOLS
...
def generate_manifest(
    ...
    alongside the ``FMP_MCP_TOOL_NAME_STYLE=spec`` setting that lets both sides
    coexist, so nothing is dropped silently.
    ...
    # Build tool list
    if tools is None:
        selected_tools = _startable_catalog(available_specs, excluded)
    else:
    ...
    manifest_content = _manifest_header(excluded, collisions) + "\nTOOLS = [\n"
